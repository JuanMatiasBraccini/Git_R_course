...
#Solution
# 1. Read in "Lesson4_exercise1.csv" 
Dat=read.csv("C:/Matias/Cursos/2019_Mexico/Data.sets/Lesson4_exercise1.csv")

# 2. Fit logistic model
Model1 <- glm(Mature~Length,family=binomial,data=Dat)

# 3. Extract summary, and % deviance explained (this  
#     is (null.deviance-model deviance)/null.deviance
  #summary
summary(Model1)
  
  #Percentage deviance explained by model
100*(Model1$null.deviance-Model1$deviance)/Model1$null.deviance

# 4. Get model predictions for the size range 100:200
Pred.size=100:200
predictions.SE = predict(Model1,newdata=list(Length=Pred.size),
                         type="r",se.fit = TRUE)

# 5. Plot predictions and 95% confidence 
#     intervals (as a blue filled polygon)
Upper.95=predictions.SE$fit+1.96*predictions.SE$se.fit
Lower.95=predictions.SE$fit-1.96*predictions.SE$se.fit

plot(Pred.size,predictions.SE$fit,type='l',
     ylab="Proportion",xlab="Lengt")
polygon(c(Pred.size,rev(Pred.size)),
        c(Lower.95,rev(Upper.95)),col=rgb(.1,.1,1,alpha=.2))
```
