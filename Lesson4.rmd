...
par(mfcol=c(3,1),mar=c(4,4,.1,.1),oma=c(2,2,.1,.1),
    mgp=c(2,.8,0),cex.lab=1.5)   #what is par? type ?par
plot(predictionpoints,predictions,xlab="",ylab="Proportion mature",
...
# This function needs to calculate, in this case, the probability  
#  of fish being mature, given their associated lengths, and the 
#  negative log-likelihood (objective function value),
#  associated with the data and different sets of maturity
#  parameter values
...
```
## by the way.... what is a function???
* A function is a set of statements organized together to perform a specific task
* R has a large number of **in-built functions** (e.g. *mean()*) and the user can **create** their own functions.
...
# When using the second formuala, we get a problem if the L95 value 
#  is ever equal to or below the L50 value. Let's see what happens
L50 <- 300
L95 <- 250
ProbOfMaturity <- 1 / (1 + exp(-log(19) * (xVals - L50) / (L95 - L50)))
...
# There is never this particular problem when using the formula 
#  containing the slope and L50. 
