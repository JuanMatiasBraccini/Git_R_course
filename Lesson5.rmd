...
```{r}
#read in data
d=read.csv(paste(hndl,
              "Lesson5_fisheries catch and effort data v2.csv",sep=""))

#mean and SD of total catches by year and month
d1.tap=with(d, tapply(catch, list(year, month), mean,na.rm=T))
head(d1.tap,2)

d2.tap=with(d, tapply(catch, list(year, month), mean,sd=T))
head(d2.tap,2)

d1.ag=aggregate(catch~year+month,d, mean,na.rm=T)
head(d1.ag,2)

d2.ag=aggregate(catch~year+month,d, sd,na.rm=T)
head(d2.ag,2)

#average catch rate by year and vessel
d.cpue = d %>%
        filter(!is.na(catch)) %>%
        mutate(cpue=catch/days)%>%
        group_by(vessel) %>%
        summarise(cpue.mean=mean(cpue), 
                  cpue.sd=sd(cpue), 
                  n=n()) %>%
        mutate(cpue.se=cpue.sd/sqrt(n), 
               lowCL=cpue.mean-1.96*cpue.se, 
...
#plot them
with(d.cpue,plot(1:length(vessel),cpue.mean,ylab="CPUE",
          xlab="Vessel",pch=19,col=3,cex=1.25,xaxt='n',
          ylim=c(0,max(uppCL))))
with(d.cpue,segments(1:length(vessel),lowCL,
                     1:length(vessel),uppCL ,col=3,lwd=2))
axis(1,1:length(d.cpue$vessel),d.cpue$vessel)
