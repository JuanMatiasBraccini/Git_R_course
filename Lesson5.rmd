...
# the function strptime() strips a date out of a character string by assuming 
# the date is encoded according to the format you specify in "quotes". The %
# sign preceeds a single letter code for what variable is next and then after
# that is whatever else will occur in the character string before the next time
# variable. 

dates = strptime(silly.dates2,format="%m/%d/%y")  
dates
   
strptime(silly.dates,format="%m/%d/%y")   # whooops! It's putting a 2020 at the
                                          # start of each date!

# we need to tell strptime the correct format: the year is abbreviated to two
# digits. Use %Y, NOT %y (see table in Crawley's blue book (The R Book))   
strptime(silly.dates,format="%m/%d/%Y")

# Of course, getting the dates into R is just the first step. We may very well
# want to extract the year, month and day. This can be done with the substr()
# function as follows:
...
# now let's read in a text file with dates and other data. Here, the dates & 
...
attach(d)
dim(d)
# The data set consists of several columns indicating time and one response 
# variable, temperature. 

# We need to write time so that it can be handled by R.

# First let's join together in the proper format the hours, minutes and seconds
...
time.of.day.VA
# The date gets formed by grouping together year, month and day in that order, 
# separated by a dash.
...
# POSIXct gives you the number of seconds passed since Jan1, 1970 (Think ct =
#     cumulative time)
# POSIXlt gives you dates in a form that can be broken down (using the unclass()
#     function) into a list of vectors representing sec, min,hrs,day,month,year
#     (Think lt = list of time elements)
# year is scored as number of years since 1900 so that 2008 gets a score of 108
# mday = day number within the month
# mon is the month but January is scored as zero, so September is 8 not 9
# wday is the day of the week starting with Sunday with a score of 0
# yday is the day of the year starting with January 1 with a score of 0
# isdst returns whether daylight savings time is in operation

# POSIXlt function
date.complete
date1lt <- as.POSIXlt(date.complete)
date1lt            # note the very slight difference between date.complete and
                   # date1lt
unclass(date1lt)
unlist(unclass(date1lt)) # The output of the function unlist is not very 
...
unlist(unclass(date1lt[5]))    # this is a vector representing all the
...
#
# R will allow you to do some operations with time.  You can do addition and
# substraction of two times.  You can also add or substract a constant, 
# multiply or divide by a constant.
# However, multiplication of two times, power, log, etc. are not possible.  
# In order to do them, you need to reverse the time to numeric using the 
# as.numeric function.
# Logical operations such as <=, =>, etc. are also possible with time.


# How many shoppings days are left until Christmas?
Christmas <- as.POSIXlt("2008-12-25")
Christmas
today <- as.POSIXlt("2008-11-24")  # of course, you'd substitute the date you're
                                   # doing this to find out how many days YOU
                                   # have left until Christmas!
today
...
# Suppose we're really interested in the log of number of days until Christmas
log(Shopping.days)
num.days <- as.numeric(Shopping.days)
num.days
log(num.days)
# The same operations can be done with POSIXct
Christmas <- as.POSIXct("2008-12-25")
Christmas
today <- as.POSIXct("2008-11-19")
today
Christmas-today

# How many seconds until the end of class?
z1 <-as.POSIXlt("2008-11-24 16:20:00")    # you'll want to update the time!
z1
z2<- as.POSIXlt(Sys.time())
z2
z1 - z2
number.sec <- round(as.numeric((z1-z2)*3600 ),digits=0)
cat(paste("There are ",number.sec," seconds until the end of class","\n"))

# Use of the difftime function
# If J.S.Bach was alive today, how many days old would he be?
difftime("2008-11-19","1685-03-31")
age.Bach <- round(as.numeric(difftime( "2008-11-18","1685-03-31"), digits=0))
age.Bach

# How long have we been sitting in this class already?
difftime( Sys.time(), "2008-11-24 15:00:00")
...
if (decision==T) cat("\n\nRochelle is a GREAT professor")
if (decision) cat("\n\nRochelle is a GREAT professor")

if (decision==F) cat("\n\nRochelle is a GREAT professor")
if (!decision) cat("\n\nRochelle is a GREAT professor")

# We can include multiple statements in an if() construct if we enclose them
# in braces {  }.  Also, we can nest if statements
...
if(decision==1){
  ...
  for (i in 1:5) {
    ...
    A <- "Oh"
    B <- " wow!"
    Sorry <- ", Rom owes me $10"
    cat("\n",A,B,Sorry)
}
       # In case you're wondering, Rochelle and Rom are married and they are
       # both my colleagues and my good friends. But, Rom and I always tease 
       # each other by insulting each other. So, when they both audited my R
       # course I couldn't resist giving Rom a hard time.
...
sex = ifelse(values <= 4,"F","M") # if values is < or = to 4, sex is assigned 
                                  # "F"; otherwise it is assigned "M"
sex

################################################################################
# Let's contrast several ways of controlling program flow: for, while and repeat
...
fac1(3)
sapply(0:5,fac1) # don't worry about the sapply function. Just accept (for now)
...
#2. lapply
#used for applying a given function to every element of a list and obtain a list as a result
