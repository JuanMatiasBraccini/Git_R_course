...
#  i.e. model1, is sufficient ... so we also need x2
...
```{r}
###############################
#1. generate data for 2-way anova by creating vector of temperature treatments
...
thedata=read.csv(paste(hndl,'Lesson3_linear.model.exercise.csv',sep=''))


#2. fit linear model
Mod1=lm(Response ~ Pred1 + Pred2+Pred3+Factor1,data=thedata) 


#3. Look at outputs
summary(Mod1)

#4. Extract anova table
AOV=anova(Mod1)
AOV
...
```{r}
#1. read in data
dat1=read.csv(paste(hndl,'Lesson3_non.linear.model.exercise.csv',
                    sep=''))
dat2=read.csv(paste(hndl,'Lesson3_non.linear.model.exercise2.csv',
                    sep=''))


#2. Plot the data
plot(dat1$Age,dat1$Length)
plot(dat2$Age,dat2$Length)


#3. fit non linear model
mod1 = nls(Length~Linf*(1-exp(-k*(Age-to))),
           data=dat1,
           start=list(Linf=240,k=.2,to=-3))
mod2 = nls(Length~Linf*(1-exp(-k*(Age-to))),
           data=dat2,
           start=list(Linf=240,k=.2,to=-3))
  

#4. Compare model predictions 
par(mfcol=c(2,1),mar=c(4,4,1,1),mgp=c(2,.7,0))
plot(dat1$Age,dat1$Length,ylim=c(0,400),ylab="Length",xlab="Age")
...
plot(dat2$Age,dat2$Length,ylim=c(0,400),ylab="Length",xlab="Age")
Linf=coef(mod1)[1]   
k = coef(mod1)[2]
to = coef(mod1)[3]
curve(Linf*(1-exp(-k*(x-to))),0,400,add=T,col=3)
