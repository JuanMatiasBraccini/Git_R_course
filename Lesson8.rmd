...
# Lesson goals
We're going to develop and fit Leslie matrices, surplus production and age-structured population dynamics models.
...
- At each time step, the population is represented by a vector $N_t$ with an element for each class where each element indicates the number of individuals currently in that class
- The key elements are:
  ...
  - $f_x$, fecundity at class x, the number of offspring produced at the next class weighted by the probability of reaching the next age class.
...
RangeA.1=c(40,60)  #max age range
k.mean=0.056       #growth parameters
Linf.mean=7.19
Lo.mean=1.4
Range.reprod_cycle=1:3   #range of possible reproductive cycles
prob.rep.cycle=c(0.05,0.25,0.7)   # probs for 1, 2 or 3-year reprod cycle
Rangefec=2:10         #litter size
age.mat=c(9,17)      #range age at maturity of females
...
#2.  Function for calculating Natural mortality
...
mx.sim=add.missing.age(mx.sim,RangeA.1[2])
...
plot(1:RangeA.1[2],m.med,pch=19,col=1,cex=1.25,xlab="Age",
     ylab=expression("Natural mortality  " (year^-1)),
     ylim=c(0,max(m.upper)))
segments(1:RangeA.1[2],m.upper,1:RangeA.1[2],m.lower)
